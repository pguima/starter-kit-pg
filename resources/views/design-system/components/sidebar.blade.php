...
<aside
    class="fixed left-0 top-16 z-50 h-[calc(100vh-4rem)] w-60 -translate-x-full overflow-hidden border-r border-[var(--border-subtle)] bg-[var(--surface-sidebar)] transition-[transform,width] duration-200 ease-out lg:sticky lg:translate-x-0"
    :class="[
    ...
    <div class="flex h-14 items-center justify-between px-4 lg:hidden">
        ...
        <button type="button"
            class="rounded-md border border-[var(--border-default)] bg-[var(--surface-hover)] px-2 py-1 text-sm text-[var(--text-primary)] hover:bg-[var(--surface-selected)]"
    ...
    @foreach($finalMenu as $group)
           <div class="mb-6">
                @if(isset($group['title']) && $group['title'])
                     <div class="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]"
                ...
                @foreach($group['items'] as $element)
                    @php
                        ...
                        $stateClass = $isActive
                            ...
                            : "text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--color-primary)]";

                        $iconWrapperClass = "inline-flex h-8 w-8 items-center justify-center rounded-md bg-[var(--surface-hover)]";
                        $iconColorClass = $isActive ? "text-[var(--color-primary)]" : "text-current group-hover:text-[var(--color-primary)]";
                    @endphp
